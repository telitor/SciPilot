...
import { CheckCircle2, ClipboardCheck, Loader2, Play, Plus, XCircle } from 'lucide-react';
...
export default function ExperimentRunPanel({
  ...
  const createRun = async () => {
    ...
    try {
      ...
      setOperatingSystem('');
      setNotes('');
      addNotification({ type: 'success', message: '实验运行记录已创建', duration: 3000 });
    } catch (error) {
      addNotification({
        type: 'error',
    ...
    <div className="sci-card">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-sci-muted">手动证据模式 · 不保存密钥</p>
        </div>
      ...
      <div className="mt-3 flex justify-end">
        <button type="button" onClick={() => void createRun()} disabled={isSaving} className="sci-btn-primary">
