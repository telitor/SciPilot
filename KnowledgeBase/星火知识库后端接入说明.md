...
XFYUN_KB_READ_TIMEOUT=120
XFYUN_KB_TOP_N=6
XFYUN_KB_RETRIEVAL_FILTER_POLICY=REGULAR
XFYUN_KB_TEMPERATURE=0.2
```

检索、状态和仓库读取使用统一的有限重试、指数退避与熔断冷却；上传、删除和 SSE 生成问答不会自动重放。共享默认值、覆盖变量和无敏感信息的运行摘要见 [`docs/EXTERNAL_SERVICE_RELIABILITY.md`](../docs/EXTERNAL_SERVICE_RELIABILITY.md)。
